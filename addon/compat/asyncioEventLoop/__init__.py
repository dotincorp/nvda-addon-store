...
Backport of NVDA 2026.2's private ``_asyncioEventLoop`` module (NVDA PR #19816),
copied verbatim so the selector in ``addon/ble/asyncUtils.py`` can swap between the
two by import alone.

.. note::
	**Delete this whole package** -- and the ``except ImportError`` fallback branch in
	``addon/ble/asyncUtils.py`` -- once ``addon_minimumNVDAVersion`` in ``buildVars.py``
	reaches 2026.2. NVDA owns the loop's lifecycle from that version on; the addon only
	starts and stops it while running on 2026.1.
"""
