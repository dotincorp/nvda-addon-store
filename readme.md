...
The add-on also asks whether to turn off NVDA's blinking braille cursor. Blinking rewrites the whole display twice a second, which the Dot Pad is too slow to do comfortably, and the add-on shows the cursor solidly anyway. NVDA has one blinking setting for all braille displays, so answering yes turns blinking off on every display you use. This question is likewise only asked once, and can be changed at any time in NVDA's braille settings, under "Blink cursor".
