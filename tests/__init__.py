...
# Enable faulthandler so any C-level crash (e.g., STATUS_STACK_OVERFLOW at
# interpreter shutdown when winrt / bleak / COM cleanup races on Python 3.13)
# emits a Python traceback to stderr instead of just a process exit code.
# Costs nothing on success runs; gives us actionable diagnostics if the
# intermittent stack overflow on NVDA 2026.1 (Python 3.13 x64) reproduces.
faulthandler.enable()

# The path to the unit tests.
UNIT_DIR = os.path.dirname(os.path.abspath(__file__))
# The path to the addon being developed
ADDON_DIR = os.path.join(os.path.dirname(UNIT_DIR), "addon")
# The path to the top of the NVDA repo.
# Can be overridden via NVDA_REPO_DIR environment variable (useful for worktrees)
TOP_DIR = os.environ.get("NVDA_REPO_DIR") or os.path.join(os.path.dirname(os.path.dirname(UNIT_DIR)), "nvda")
...
from .objectProvider import NVDAObjectWithRole, PlaceholderNVDAObject  # noqa: E402, F401
...
# Allow code to check if we are under unittst
os.environ["UNDER_UNITTEST"] = "1"

# Initialize hwio to have a working bgThread
import hwIo  # noqa: E402

hwIo.initialize()

# Patch addon paths to allow addonHandler to reat this as an addon
import addonHandler  # noqa: E402

manifest = addonHandler.AddonManifest(
	StringIO("""
name = dotPad
version = 1.0
minimumNVDAVersion = 2023.1
lastTestedNVDAVersion = 9999.9999
"""),
)


class FakeAddon(addonHandler.AddonBase):
	@property
	def manifest(self):
		return manifest
	...
	def loadModule(self, moduleName):
		return importlib.import_module(f"addon.{moduleName}")

	def getTranslationsInstance(self, domain="nvda"):
		"""Return a fallback NullTranslations — addon code calls
		``addonHandler.initTranslation()`` which dispatches here. NVDA 2026.1+
		moved the production implementation onto the concrete ``Addon`` class
		(not ``AddonBase``), so the test fake needs to provide its own.
		"""
		import gettext
...
addonHandler._getDefaultAddonPaths = lambda: [os.path.dirname(ADDON_DIR)]
addonHandler._availableAddons[ADDON_DIR] = FakeAddon()
